...
export default function CartPage() {
  ...
  const getProductName = (itemId: string) => {
    const productId = itemId.split('-')[0]; // Extract product ID from cart item ID
    const product = products?.find(p => p.id === productId);
    return product?.name || items.find(i => i.id === itemId)?.name || '';
  };

  const subtotal = items.reduce((sum, item) => sum + item.price * item.weight * item.quantity, 0);
  const deliveryFee = 0; // Removed delivery fee
              ...
              <div className="flex gap-6 flex-wrap">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground mb-1">
                    ₹{item.price.toFixed(2)}/kg × {item.weight}kg
                  </p>
